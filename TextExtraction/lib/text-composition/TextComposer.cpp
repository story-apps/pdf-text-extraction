 ...
 */
static bool ParagraphContinue(const std::string& inLastWrittenText,
    ...
    for (; itCommands != sortedTextCommands.end(); ++itCommands) {
        ...
        if (AreSameLine(latestItem, *itCommands)) {
            ...
            if (firstWritableItem) {
                paragraphContinue = ParagraphContinue(lastWrittenText, lastWrittenTextBox, *itCommands);
                if (!paragraphContinue) {
